...
exit_on_failure "Unable to backup volume $VOL_NAME"
...
EXTNET_ID=$(neutron net-show public | awk '/ id /{print $4}')
...
FIP_ADD=$(neutron floatingip-create public | awk '/ floating_ip_address /{print $4}')
...
neutron security-group-rule-create --direction ingress --protocol TCP --port-range-min 22 --port-range-max 22 --remote-ip-prefix 0.0.0.0/24 $SECGRP_ID
...
# Looking for a cirros image and getting its ID
IMAGE_ID=$(nova image-list | awk '/ cirros-0.3.2-x86_64-uec /{print $2}')
exit_if_empty $IMAGE_ID "Image cirros-0.3.2-x86_64-uec could not be found"
...
# Upload glance image
glance image-create --name $IMG_NAME --disk-format qcow2 --container-format bare --file zero_disk.qcow2
...
while [ $VM_STATUS != "ACTIVE" ]; do
    echo "Status of VM $VM_NAME is $VM_STATUS. Wating 1 sec"
    sleep 1
    VM_STATUS=$(nova show $VM_ID | awk '/ status /{print $4}')
done
